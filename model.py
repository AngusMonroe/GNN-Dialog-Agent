...
class Attn(nn.Module):
    ...
    def forward(self, hidden, encoder_outputs, gnn_state):
...
class LuongAttnDecoderRNN(nn.Module):
    ...
    def forward(self, input_step, last_hidden, encoder_outputs, gnn_state):
        ...
        attn_weights = self.attn(rnn_output, encoder_outputs, gnn_state)
...
class Seq2Seq(nn.Module):
    def __init__(self, encoder, decoder, net, opts):
    ...
    def forward(self, input_variable, lengths, batch_size, teacher_forcing_ratio, max_target_len, target_variable, mask,
        ...
        gnn_state = self.net(init_input, adj_matrix)
        ...
        if use_teacher_forcing:
            for t in range(max_target_len):
                decoder_output, decoder_hidden = self.decoder(
                    decoder_input, decoder_hidden, encoder_outputs, gnn_state
                )
